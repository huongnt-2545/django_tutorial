...
class AuthorListViewTest(TestCase):
    ...
    def test_view_uses_correct_template(self):
        response = self.client.get(reverse("authors"))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "authors/index.html")
...
class LoanedBookInstancesByUserListViewTest(TestCase):
    def setUp(self):
        user1 = User.objects.create_user(username="user1", password="1234@1234")
        user2 = User.objects.create_user(username="user2", password="1234@1234")
    ...
    def test_logged_in_uses_correct_template(self):
        login = self.client.login(username="user1", password="1234@1234")
        response = self.client.get(reverse("my-borrowed"))

        self.assertEqual(str(response.context["user"]), "user1")
        self.assertEqual(response.status_code, 200)

        self.assertTemplateUsed(response, "bookinstances/list_borrowed_user.html")
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_forbidden_if_logged_in_but_not_correct_permission(self):
        login = self.client.login(username="user1", password="1234@1234")
        response = self.client.get(
            reverse("renew-book-librarian", kwargs={"pk": self.test_bookinstance1.id})
        )
    ...
    def test_logged_in_with_permission_borrowed_book(self):
        ...
        response = self.client.get(
            reverse("renew-book-librarian", kwargs={"pk": self.test_bookinstance2.id})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_HTTP404_for_invalid_book_if_logged_in(self):
        ...
        login = self.client.login(username="user2", password="1234@1234")
        response = self.client.get(
            reverse("renew-book-librarian", kwargs={"pk": test_uuid})
        )
    ...
    def test_uses_correct_template(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "bookinstances/book_renew_librarian.html")
    ...
    def test_redirects_to_all_borrowed_book_list_on_success(self):
        ...
        self.assertRedirects(response, reverse("list-book-borrowed"))
...
class AuthorCreateViewTest(TestCase):
    ...
    def test_forbidden_if_user_logged_in_but_not_correct_permission(self):
        login = self.client.login(username="user1", password="1234@1234")
        response = self.client.get(reverse("author-create"))

        self.assertEqual(response.status_code, 403)

    def test_user_logged_in_and_have_permission(self):
        login = self.client.login(username="user2", password="1234@1234")
        response = self.client.get(reverse("author-create"))
    ...
    def test_uses_correct_template(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "authors/new.html")

    def test_form_date_death_initially_default(self):
        login = self.client.login(username="user2", password="1234@1234")
        response = self.client.get(reverse("author-create"))

        self.assertEqual(response.status_code, 200)
        default_date = "11/06/2020"
